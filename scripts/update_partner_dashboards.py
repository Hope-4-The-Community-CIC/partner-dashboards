...
SKIP_DIRS = {
    ".git", ".github", "scripts", "node_modules", "assets",
...
def tidy_waitlist_name(raw_name, mappings):
    ...
    low = name.lower()
    # Friendly generic fallback for the common Qualtrics wording.
    if "waitlist" in low:
        name = re.sub(r"^waitlist\s*-\s*", "", name, flags=re.I).strip()
        if name:
...
def update_partner(folder):
    ...
    csv_url = (config.get("qualtricsCsvUrl") or "").strip()
    if not csv_url:
    ...
    required = {"Quota Name", "Quota Count", "Quota Target"}
    ...
    # Default Thursday (0=Mon ... 3=Thu ... 6=Sun), configurable per partner.
    snapshot_weekday = int(config.get("weeklySnapshotWeekday", 3))
    is_snapshot_day = now.date().weekday() == snapshot_weekday
    ...
    waitlist_mappings = config.get("waitlistLabels") or {}
    new_waitlists = []
    changed = False

    for row in rows:
        quota_name = (row.get("Quota Name") or "").strip()
        count = as_int(row.get("Quota Count"))
        target = as_int(row.get("Quota Target"))

        if "waitlist" in quota_name.lower():
            new_waitlists.append({
                "name": tidy_waitlist_name(quota_name, waitlist_mappings),
                "count": count
            })
            continue

        route_date = parse_uk_date(quota_name)
        if not route_date:
            # Ignore non-date, non-waitlist quotas.
            continue

        route = group_by_date.get(route_date)
        if route is None:
            route = {
        ...
        if as_int(route.get("count")) != count or as_int(route.get("target")) != target:
            changed = True
        ...
        # Keep weekly uptake weekly rather than creating 15-minute/daily points.
        history = route.setdefault("history", [])
        if is_snapshot_day:
            if history and history[-1].get("date") == today:
                if as_int(history[-1].get("count")) != count:
...
def main():
    ...
    for folder in sorted(ROOT.iterdir()):
        ...
        if not (folder / "config.json").exists() or not (folder / "data.json").exists():
            continue

        found += 1
        try:
            changed, message = update_partner(folder)
            if changed:
                changed_count += 1
            print(f"{folder.name}: {message}")
        except Exception as exc:
            # Fail the workflow if a configured partner cannot update.
            raise RuntimeError(f"Failed updating {folder.name}: {exc}") from exc

    if found == 0:
        raise SystemExit("No partner dashboard folders were found.")

    print(f"Checked {found} partner dashboard(s); changed {changed_count}.")
